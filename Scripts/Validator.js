...
//-----------------------------------------used for textbox maxlength validaion-------------------------
function MaxLenTxtValid(ctr, lbl, maxLen) {
    $("#" + ctr).attr('maxlength', maxLen);
    var left = maxLen - $("#" + ctr).val().length;
    if (left < 0) {
        left = 0;
    }
    $("#"+lbl).text(left);
...
function checkjpeg1(ctr) {
    debugger;
    if (document.getElementById(ctr).value != '') {
        debugger;
        var myString = document.getElementById(ctr).value;
        var myString = myString;

        var ext = myString.substring(myString.lastIndexOf(".") + 1);

        if ($.inArray(ext, ['jpg', 'JPG', 'png', 'JPEG']) == -1) {
            CustomValidation(ctr, 'Only .jpg file applicable.');
...
//-----------------------------------------used to check pdfformat of a file-------------------------
function checkpdf(ctr) {
    debugger;
    if (document.getElementById(ctr).value != '') {
        var ext = $("#" + ctr).val().split('.').pop().toLowerCase();
        if ($.inArray(ext, ['pdf', 'PDF']) == -1) {
            CustomValidation(ctr, 'Only .pdf file Applicable!');
            $(this).val('');           
            return false;
        }
        if ((this.files[0].size > 2097152) && ($(this).val() != '')) {
            CustomValidation(ctr, 'File size must be less then 2mb!');
            $(this).val('');
            return false;
        }
...
//-----------------------------------used to check deley reasion--------------------------------
function checkdeleyreasion(ctr,hdnfld) {
    var rejectdt = new Date($("#"+hdnfld).val());
    var today = new Date();
    var timeDiff = Math.abs(today.getTime() - rejectdt.getTime());
    var diffDays = Math.ceil(timeDiff / (1000 * 3600 * 24));
    if (diffDays > 30) {
        if ($("#" + ctr).val() == '') {
            alert('Please enter the delay reasondffd.');
            // $("#" + ctr).focus();
            $(this).focus();
            return false;
        }
...
function chkresuduleddate(ctr) {

    {
        ...
        var date2 = new Date();
        if (date1 < date2) {
            alert('Please enter reschedule date greater than current date');
            //$("#" + ctr).focus();
            $(this).focus();
            return false;
        }
...
function chkresuduleddate1(ctr) {
    {
        var ddate = $("#" + ctr).val();
        var date1 = new Date(ddate);
        var date2 = new Date();
        if (date1.getDate() != date2.getDate()) {
            alert('Reject date should be current date.');
            $("#" + ctr).focus();
            return false;
        }
    }
...
function chkresuduleddate2(ctr) {
    {
        var ddate = $("#" + ctr).val();
        var date1 = new Date(ddate);
        var date2 = new Date();
        if (date1.getDate() != date2.getDate()) {
            alert('Approve date should be current date.');
            $("#" + ctr).focus();
            return false;
...
//------------------label text changep--------------------- $("#Lbldate").text("Approve Date");
function labeltextchange(lbl,msg) {
    {
        $("#"+lbl).text(msg);
    }
